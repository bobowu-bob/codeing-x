...
public class Player {
	...
	private ImageIcon ib = new ImageIcon("img\\bullet\\normal.png");
	...
	public void locataDirection() {
		if (bL && !bU && !bR && !bD)
			dir = Direction.L;// 嚙踝�蕭謘蕭嚙踐�蕭�瞏叟�蕭嚙踝蕭
		else if (bL && bU && !bR && !bD)
			dir = Direction.LU;
		else if (!bL && bU && !bR && !bD)
			dir = Direction.U;
	...
	public void move() {
		...
		int[][] collision_array = Map.sendmap();	//get the map data
		
		switch (dir) {
		case L:
			if (x > 0 && !(collision_array[x_left][y / 48] == 2 
			     || collision_array[x_left][(y + 36) / 48] == 2
				 || collision_array[x_left][y / 48] == 3 
				 || collision_array[x_left][(y + 36) / 48] == 3))
				x -= XSPEED;
			inow = il;
			break;
		...
		case RU:
			...
			break;
		case R:
			if (x < 588 && !(collision_array[x_right][y / 48] == 2 || collision_array[x_right][(y + 36) / 48] == 2
					|| collision_array[x_right][y / 48] == 3 || collision_array[x_right][(y + 36) / 48] == 3))
				x += XSPEED;
			inow = ir;
			break;
		case RD:
			...
			break;
		case D:
			if (y < 610 && !(collision_array[x / 48][y_down] == 2 || collision_array[(x + 36) / 48][y_down] == 2
					|| collision_array[x / 48][y_down] == 3 || collision_array[(x + 36) / 48][y_down] == 3))
				y += YSPEED;
			inow = id;
			break;
